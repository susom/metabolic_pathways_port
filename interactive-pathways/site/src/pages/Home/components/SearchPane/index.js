...
const { nextButtonId, plotLayerId, previousButtonId, svgId } = homeConfig;
...
const zoomAllMapPinIcons = () => {
  ...
  const scale = Math.max(1, Math.min(config.maxZoomScale, 0.9 / Math.max(dx / width, dy / height)));
...
class SearchPane extends Component {
  ...
  nextPin = () => {
    ...
    const { x, y } = R.prop(id, textCollection);
  ...
  plotAllPins = (data) => {
    ...
    let mapPin = this.plotLayer.selectAll(`g.${plotResultClassName}`)
        ...
        .attr(
            'transform',
            ({ x, y, }) =>
                `matrix(1 0 0 1 ${parseFloat(x) + config.xOffset} ${parseFloat(y) + config.yOffset})`
        )
